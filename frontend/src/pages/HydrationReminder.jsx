...
import { Droplet, Bell, Clock, ArrowLeft, CheckCircle } from "lucide-react";
...
export default function HydrationReminder() {
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        ...
        setIntervalMins(s.intervalMins ?? 120);
        setOfficeStart(s.officeStart ?? 9);
        setOfficeEnd(s.officeEnd ?? 17);
  ...
  useEffect(() => {
    ...
    try {
      ...
      const s = { enabled, intervalMins, officeStart, officeEnd, useNotifications, weekdaysOnly, customMessage, nextAt };
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
            <div className="rounded-xl border p-4">
              ...
              <input
                ...
                step={15}
                value={intervalMins}
                onChange={(e) => setIntervalMins(Number(e.target.value || 0))}
                className="w-full rounded-lg border px-3 py-2 text-base sm:text-sm"
                aria-describedby="interval-help"
              />
              <p id="interval-help" className="mt-1 text-xs text-slate-500">Minimum 15 minutes, step 15 minutes</p>
            </div>
            ...
            <div className="rounded-xl border p-4">
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  max={23}
                  value={officeStart}
                  onChange={(e) => setOfficeStart(Number(e.target.value || 0))}
                ...
                <input
                  ...
                  max={24}
                  value={officeEnd}
                  onChange={(e) => setOfficeEnd(Number(e.target.value || 0))}
                  className="w-20 rounded-lg border px-3 py-2 text-base sm:text-sm"
                  aria-label="End hour"
                />
              </div>
              <p className="mt-2 text-xs text-slate-500">Example: 9 to 17 covers 9:00–17:00.</p>
            </div>
          </div>
